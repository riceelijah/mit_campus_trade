...
db.exec(`
    ...
    -- Snapshots of a conversation_notes value the instant before it's overwritten by a
    -- student edit (see setExchangeEventConversationNotes) -- lets admin see a note's full
    -- edit history for research-integrity purposes, even though students only ever see/edit
    -- the current text on the My Notes page.
    CREATE TABLE IF NOT EXISTS conversation_note_revisions (
        revision_id INTEGER PRIMARY KEY AUTOINCREMENT,
        exchange_id INTEGER NOT NULL REFERENCES exchange_events(exchange_id),
        notes TEXT NOT NULL,
        replaced_at TEXT NOT NULL DEFAULT (datetime('now'))
    );
    CREATE INDEX IF NOT EXISTS idx_conversation_note_revisions_exchange_id
        ON conversation_note_revisions(exchange_id);
...
/** One prior version of an exchange_events row's conversation_notes, archived the instant it
 *  was overwritten by a student edit -- see setExchangeEventConversationNotes. */
export interface ConversationNoteRevisionRow {
    revision_id: number;
    exchange_id: number;
    notes: string;
    replaced_at: string;
...
const stmts = {
    ...
    // Same as listExchangeEvents, scoped to one user -- powers the self-service My Notes page.
    listExchangeEventsForUser: db.prepare(
        'SELECT * FROM exchange_events WHERE user_id = ? ORDER BY exchange_id DESC',
    ),
    insertConversationNoteRevision: db.prepare(
        'INSERT INTO conversation_note_revisions (exchange_id, notes) VALUES (?, ?)',
    ),
    listNoteRevisionsForExchangeEvent: db.prepare(
        'SELECT * FROM conversation_note_revisions WHERE exchange_id = ? ORDER BY revision_id ASC',
    ),
    ...
    `),
    // Same rows as custodyForCardInstance, but unjoined -- callers that need to correlate a
    // custody transition back to the exact exchange_id that caused it (see
    // removalEventsForUser in serialize.ts, matching a removal to the trade that caused it)
    // need exchange_id itself, which the joined/renamed CustodyEventWithUser shape drops.
    exchangeEventsForCardInstance: db.prepare(
        'SELECT * FROM exchange_events WHERE card_instance_id = ? ORDER BY exchange_id ASC',
    ),
...
/** For recognizing a denormalized received_card_previous_user_email as the reserved
 *  Unassigned account (e.g. My Notes deciding not to show "From: Unassigned" to a student --
 *  see buildMyCardEventsFeed in serialize.ts) where only the email, not a user id, is on hand. */
export function unassignedUserEmail(): string {
    return unassignedUser.email;
}
...
/** Same history as custodyForCardInstance, as plain exchange_events rows (own exchange_id
 *  included) rather than joined/renamed for display -- see that statement's own comment. */
export function exchangeEventsForCardInstance(cardInstanceId: number): ExchangeEventRow[] {
    return stmts.exchangeEventsForCardInstance.all(cardInstanceId) as unknown as ExchangeEventRow[];
}
 ...
 * their own exchange events -- and, from the My Notes page, any later edit to it too. Same
 * ownership check/contract as setExchangeEventReceivedFromOther.
 *
 * Whatever the note said immediately before this call is archived into
 * conversation_note_revisions first, so an edit never actually destroys the earlier text --
 * only the very first save (conversation_notes still null) has nothing to archive. Students
 * only ever see/edit the current text; the archive is admin-only (see
...
export function setExchangeEventConversationNotes(
    ...
    const row = findExchangeEventById(exchangeEventId);
    if (!row || row.user_id !== userId) return false;
    if (row.conversation_notes !== null) {
        stmts.insertConversationNoteRevision.run(exchangeEventId, row.conversation_notes);
    }
...
/** Every card-obtained event belonging to `userId`, current or historical -- powers the
 *  self-service My Notes page (GET /api/me/card-events). */
export function exchangeEventsForUser(userId: number): ExchangeEventRow[] {
    return stmts.listExchangeEventsForUser.all(userId) as unknown as ExchangeEventRow[];
}

/** Every earlier version of one exchange event's conversation_notes, oldest first -- admin-only
 *  (see setExchangeEventConversationNotes); students never see their own edit history. */
export function noteRevisionsForExchangeEvent(exchangeEventId: number): ConversationNoteRevisionRow[] {
    return stmts.listNoteRevisionsForExchangeEvent.all(
        exchangeEventId,
    ) as unknown as ConversationNoteRevisionRow[];
}
